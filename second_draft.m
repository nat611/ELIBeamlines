...
for f = 1:length(folders)
   ...
   % calculating diameters
    ...
    %solid angle 
    CR_area = ((395.0617*10^-6)*(246.9135*10^-6))*10;%where 10 is the number of microscope images 
    distance = 52.5*10^-2;
...
x_combined = [all_x_axis{:}]; 
y_combined = [all_y_axis{:}]; 
p = polyfit(x_combined, y_combined, 1);
y_fit = polyval(p, x_combined);
...
%mean ine
%plot(x_combined, y_fit, '-r', 'LineWidth', 2); % Line of best fit in black
